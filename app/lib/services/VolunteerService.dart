...
class VolunteerService {
    ...
    Future<List<Volunteer>> fetchVolunteers(String cafeName) async {
        var url = Uri.parse("${Config.apiUrl}/cafes/$cafeName/volunteer");
        ...
        if (response.statusCode == 200) {
            ...
            if (jsonData['volunteers'] != null) {
                ...
                List<Volunteer> volunteers = volunteersJson.map((json) => Volunteer.fromJson(json)).toList();
    ...
    Future<String> addVolunteer(String cafeSlug, String username, String role) async {
        String message = "";
        final token = await _authService.getToken();
        final response = await http.post(
        Uri.parse('${Config.apiUrl}/cafes/$cafeSlug/staff'),
        headers: {
    ...
    Future<String> deleteVolunteer(String cafeSlug, String username) async {
        ...
        final response = await http.delete(
            Uri.parse('${Config.apiUrl}/cafes/$cafeSlug/staff/$username'),
            headers: {
                'Authorization': 'Bearer $token',
                'Content-Type': 'application/json'
            });
